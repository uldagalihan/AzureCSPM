"""
Key Vault RBAC (role-based access control) inventory.

For each Key Vault listed in an input CSV, retrieves all role assignments
at the vault resource scope, resolves role definition names and principal
display names, and exports the results to CSV.

Expected input:  output/key_vaults.csv  (column: ID)
Output:          output/keyvault_rbac.csv
...
def get_role_assignments(
    ...
    """Retrieve role assignments scoped to a specific resource.

    Args:
        subscription_id: Azure subscription ID.
        rg: Resource group name.
        provider: Resource provider namespace (e.g. ``Microsoft.KeyVault``).
        resource_type: Resource type (e.g. ``vaults``).
...
def get_role_definition(role_def_id: str, token: str, cache: dict) -> tuple:
    ...
    # The disambiguation_dummy parameter avoids a known ARM caching quirk.
...
def save_to_csv(data: list, filepath: str) -> None:
    """Write Key Vault RBAC records to a CSV file.
    ...
    """
    fieldnames = [
        "keyvault_id", "role_definition_id", "role_name", "description",
...
def main() -> None:
    """Entry point: collect RBAC assignments for all Key Vaults in the input CSV."""
    ...
    output_path = os.path.join(OUTPUT_DIR, "keyvault_rbac.csv")
    ...
    for kv_id in kv_ids:
        sub, rg, provider, resource_type, resource_name = parse_arm_id(kv_id)
        if not all([sub, rg, provider, resource_type, resource_name]):
            logger.warning("Could not parse Key Vault ARM ID: %s", kv_id)
            continue

        logger.info("Fetching role assignments for: %s", resource_name)
        role_assignments = get_role_assignments(
            sub, rg, provider, resource_type, resource_name, mgmt_token
        )
